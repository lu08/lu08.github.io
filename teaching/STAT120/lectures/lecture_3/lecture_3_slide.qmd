...
-   For **numeric variables**, use mean and/or median
    ...
    -   When mean and median are very different, it signals very **skewed** distribution and/or existence of outliers
...
![](percentile_example.jpg){width=30%}

-   The kth percentile of a variable is a value such that k percent are below that value

    -   Example: [Growth charts for children](https://www.cdc.gov/growthcharts/html_charts/wtageinf.htm#females)
    -   Example: [SAT scores](https://en.wikipedia.org/wiki/SAT)
...
- Percentiles (especially Q1, Q2, and Q3) also provide a sense of the **location** of the distribution
...
-   For determining the direction of skewness, look for **tail values**; extreme tail values are referred to as **outliers**
    ...
    - Tail values or extreme values are LARGE values: right skewed (positive skew); e.g., income, house price, etc.
    - Tail values or extreme values are SMALL values: left skewed (negative skew); e.g., age at death, retirement age, etc.
...
```{r,echo=F,out.width="80%",fig.height=3,fig.width=9}
par(mfrow=c(1,3))
...
-   **Z-score** (also called standardized score) of a variable is calculated as `(value - mean) / sd`
...
# using scale function: a matrix is created
